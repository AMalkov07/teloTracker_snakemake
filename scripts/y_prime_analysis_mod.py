...
input_blast_dir = sys.argv[3]
output_summary_tsv = sys.argv[4]
output_stats_txt = sys.argv[5]
output_figs_dir = sys.argv[6]

# Create the figure directory and any required subfolders immediately
...
def y_prime_count_violin_strip_plot(dataframe, section='all', x_plot='chr_end', y_plot='y_primes_relative_to_ref', plot_scale=(22,9)):
    ...
    if '6991' in base_name: # WildType
        #figure_directory = f'{output_figs_dir}/wild_type/'
        figure_directory = f'{output_figs_dir}/'
    elif '7021' in base_name: # rad52
        figure_directory = f'{output_figs_dir}/rad52/'
    elif '7093' in base_name: # rad52-annealing
        figure_directory = f'{output_figs_dir}/rad52-annealing/'
    elif '7154' in base_name: # rad51
        figure_directory = f'{output_figs_dir}/rad51/'
    elif '7172' in base_name: # mph1
        figure_directory = f'{output_figs_dir}/mph1/'
    elif '7174' in base_name: # rad59
        figure_directory = f'{output_figs_dir}/rad59/'
    elif '7250' in base_name: # rad54
        figure_directory = f'{output_figs_dir}/rad54/'
    elif '7321' in base_name: #pif1-m2
        figure_directory = f'{output_figs_dir}/pif1_m2/'
    elif '7323' in base_name: #pif1-NLS
        figure_directory = f'{output_figs_dir}/pif1_nls/'
    elif '7324' in base_name: # rif1rif2
        figure_directory = f'{output_figs_dir}/rif1rif2/'
...
if __name__ == "__main__":
    ...
    #y_prime_blast_dir=f'{base_name}/y_prime_blast_results_from_chr_anchored_reads'

    #y_prime_blast_dir_y_primes_mega=f'{y_prime_blast_dir}/y_primes_mega'
    #y_prime_blast_dir_probe=f'{y_prime_blast_dir}/probes'

    #top_anchor_blast_input=f'{base_name}/top_matches_{base_name}_blasted_{anchor_set}.tsv'
    telomere_repeat_results_input=f'results/{base_name}/{base_name}_post_telo_trimming.tsv'

    output_tsv_f_name=f'{input_blast_dir}/all_{base_name}_probe_matches.tsv'
    #new_output_file=f'outputs/{base_name}_post_y_prime_probe.tsv'

    print(f"Starting y_prime_analysis.py for {base_name}")

    probe_blasting_files = os.listdir(f"{input_blast_dir}/")
    files_to_analyze_list = [f'{input_blast_dir}/{f}' for f in probe_blasting_files if f.endswith('blasted_probe.tsv')]   # y_primes_for_blasting.tsv
    ...
    # Make files
    df_combined_results.to_csv(output_summary_tsv, sep="\t", index=False)
    ...
    # USE THE FILENAME PROVIDED BY SNAKEMAKE
    with open(output_stats_txt, 'w') as f_out:
        f_out.write(f"Y-Prime Analysis for {base_name}\n")
        f_out.write("="*30 + "\n\n")
    
        f_out.write("Stats of Neutral Sign (same):\n")
        f_out.write(stats_of_neutral_sign.to_string() + "\n\n")
    
        f_out.write("Stats of Positive Sign (+):\n")
        f_out.write(stats_of_postive_sign.to_string() + "\n\n")
    
        f_out.write("Stats of Negative Sign (-):\n")
        f_out.write(stats_of_negative_sign.to_string() + "\n")

    #print(f'Writting {output_stats_txt}')
    #with open (output_stats_txt) as f:
