# make_spacer_repeatmasker_tsv.py
...
print("Starting make_spacer_repeatmasker_tsv.py")
...
for base_name in input_base_name:
    ...
    bed_file_of_features = f"{strain}_final_features.bed"

    repeatmasker_results_dir = f'/Shared/malkova_lab/Ivan/nanopore_sequencing/telomere_analysis/working_new_telomere_analysis/{base_name}/spacer_repeatmasker_results/'
    repeatmasker_results_dir_list = os.listdir(f'/Shared/malkova_lab/Ivan/nanopore_sequencing/telomere_analysis/working_new_telomere_analysis/{base_name}/spacer_repeatmasker_results/')
    ...
    def label_anchor_on_chopped_segments(chr_end, section_number):
    ...
    df_all_repeatmakser_results['anchor_label'] = df_all_repeatmakser_results.apply(
        lambda row: label_anchor_on_chopped_segments(row['original_chr_end_anchor'], row['section_number']), axis=1)


    df_all_repeatmakser_results.sort_values(by=['original_chr_end_anchor', 'read_id', 'match_start_on_read'], inplace=True)

    df_all_repeatmakser_results.to_csv(f'{base_name}_spacer_repeatmasker.tsv' , sep= '\t')
    ...
    df_good_repeatmakser_results.to_csv(f'{base_name}_good_spacer_repeatmasker.tsv' , sep= '\t')
    ...
    df_good_gained_y_repeatmakser_results.to_csv(f'{base_name}_good_gained_spacer_repeatmasker.tsv' , sep= '\t')