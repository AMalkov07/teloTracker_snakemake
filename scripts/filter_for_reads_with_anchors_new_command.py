...
#anchor_blast_input = f'{base_name}_blasted_{anchor_set}_new_command'
...
# Make a subset of the columns
df_testing = df_unique[['read_id', 'total_read_length', 'read_bp_used_for_match', 'anchor_name', 'pident', 'bitscore', 'match_length', 'read_length_past_anchor']]

# Print the column names
#print(df_unique.columns)

# Check for multiple anchors per read
# For each read_id, filter reads if they have 'bitscore' too close to the top match

# Create a threshold for bitscore difference
bitscore_threshold = 1000  # Adjust this threshold based on your needs

# Initialize counters for tracking
one_anchor_reads_count = 0
multiple_anchor_reads_count = 0
close_entries_count = 0
close_entries_and_low_bitscore_count = 0
unique_entries_and_low_bitscore_count = 0
df_new = pd.DataFrame()

# For each unique read_id, filter the entries
for read_id in df_testing['read_id'].unique():
    read_entries = df_testing[df_testing['read_id'] == read_id]
    
    # Get the highest bitscore for this read_id
    max_bitscore = read_entries['bitscore'].max()

    if len(read_entries) > 1:
        multiple_anchor_reads_count += 1
        
        # Find all entries where the bitscore is too close to the max_bitscore
        close_entries = read_entries[read_entries['bitscore'] >= (max_bitscore - bitscore_threshold)]
        
        # If no entries are close in bitscore, keep the entire group
        if len(close_entries) <= 1:
            df_new = pd.concat([df_new, read_entries])
        else:
            close_entries_count += 1
            print(read_entries)

            if max_bitscore < 6000:
                close_entries_and_low_bitscore_count += 1
                #print(read_entries)
        
    else:
        one_anchor_reads_count += 1

        if max_bitscore >= 6000:
            df_new = pd.concat([df_new, read_entries])
        else:
            unique_entries_and_low_bitscore_count += 1
            #print(read_entries)

# Print the results
print('One anchor reads count:', one_anchor_reads_count)
print('Multiple anchor reads count:', multiple_anchor_reads_count)
print('Close entries count:', close_entries_count)
print('Close entries and low bitscore count:', close_entries_and_low_bitscore_count)
print('Unique entries and low bitscore count:', unique_entries_and_low_bitscore_count)


##################################################### keep=False or keep='first' #####################################################
...
df_new.drop_duplicates(subset=['read_id'], keep='first', inplace=True)
print('df_new')
print(df_new)
print(df_new['anchor_name'].value_counts())
print('')
