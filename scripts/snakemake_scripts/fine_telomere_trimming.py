...
input_reads_f_name = sys.argv[1]
tag_and_adapter_info_f_name = sys.argv[2]
best_anchor_telo_f_name = sys.argv[3]
output_tsv_f_name = sys.argv[4]
repeat_trim_dir = sys.argv[5]
base_name = sys.argv[6]
output_final_fasta = sys.argv[7]
...
#input_reads_f_name=f'{base_name}/{base_name}_all_chromosome_anchored_reads_pre_trim.fasta'

#tag_and_adapter_info_f_name=f'{base_name}/{base_name}_adapter_trimming_check.tsv'
#best_anchor_telo_f_name=f'{base_name}/top_matches_{base_name}_blasted_{anchor_set}.tsv'

#output_tsv_f_name=f'{base_name}/{base_name}_post_telo_trimming.tsv'

#repeat_trim_dir = f'{base_name}/repeat_trim_files/'
...
output_no_telo_repeat_fasta = os.path.join(repeat_trim_dir, f'{base_name}_final_no_telo_repeat_reads_best.fasta')
output_telo_repeat_fasta = os.path.join(repeat_trim_dir, f'{base_name}_final_telomere_repeat_reads_best.fasta')
output_outside_trim_tsv = os.path.join(repeat_trim_dir, f'{base_name}_final_telomere_repeat_outside_trim_reads_best.tsv')
...
with open(output_no_telo_repeat_fasta, "w") as f_out:
    f_out.writelines(running_both_no_telo_repeat_reads_list)

with open(output_telo_repeat_fasta, "w") as f_out:
    f_out.writelines(running_both_to_telomere_reads_list)

with open(output_outside_trim_tsv, "w") as f_out:
    f_out.writelines(running_outside_trimmed_reads_list)
...
#added coce
print(f"Combining results into final FASTA: {output_final_fasta}")

# Combine the two lists of reads you've been building in your loop
# (running_both_no_telo_repeat_reads_list and running_both_to_telomere_reads_list)
all_final_reads = running_both_no_telo_repeat_reads_list + running_both_to_telomere_reads_list

with open(output_final_fasta, "w") as f_out:
    f_out.writelines(all_final_reads)

print("Successfully wrote final trimmed FASTA.")