...
**No, not initially.** The `ws class` column in the master table above was a value carried over
from an earlier session's `cut99_summary_6991.tsv`, itself generated by `flank_identities.py`
(a related but different half-split BLAST comparison, seeded by a sliding-window-located
junction) -- not a fresh run of `scan_recombinant_junctions.py` on these specific 24 reads for
this specific claim. That is a fair thing to be skeptical of, so the actual scanner
(`scan_recombinant_junctions.py`, unmodified, `--window 300 --step 150`) was rerun from scratch,
independently, on all 58 reads just now:

| chr_end | read_id | gap | fresh scan verdict | fresh scan pattern |
|---|---|---|---|---|
| chr10L | SRR33298384.498901 | +3.60% | **RECOMBINANT** | `.AAA---AAAAAAAA-------AA-------BBBB-B-BBBBBB.` |
| chr10L | SRR33298384.151130 | +3.57% | **RECOMBINANT** | `.BBBBBB--BBBBBB-------A-------AAAAAAA----AAA.` |
| chr10L | SRR33298461.57209 | +3.39% | **RECOMBINANT** | `.BBBBBB-B-BBBB-------AA------AAAAAAAAA---AA..` |
| chr10L | SRR33298373.545529 | +2.56% | **RECOMBINANT** | `.AAA---AAAAAAAAA------BB-------BBBB-A-AAAAAA.` |
| chr10L | SRR33298373.491903 | +2.54% | **RECOMBINANT** | `.AAAAAA----BBBB-------BB------AAAA-AAAA--AAA.` |
| chr10L | SRR33298373.122658 | +2.37% | **RECOMBINANT** | `.BBBBB-------AB-------AA------AAA-AAAAA--AAA.` |
| chr13L | SRR33298384.220455 | +2.14% | single | `...............................AAAA-` |
| chr13L | SRR33298461.44252 | +0.03% | **RECOMBINANT** | `.------B---ABB-B------------AA----AAA-.` |
| chr14R | SRR33298373.897522 | +4.57% | **RECOMBINANT** | `.A-----A-A---------------------ABBBBA-----------` |
| chr14R | SRR33298377.726683 | +2.20% | unresolved | `.............................................` |
| chr14R | SRR33298373.1266643 | +0.95% | unresolved | `.............................................` |
| chr16L | SRR33298434.79614 | -1.38% | unresolved | `............................................` |
| chr16R | SRR33298384.538094 | -0.89% | **RECOMBINANT** | `.A-BB------BB------------------AAAA.` |
| chr16R | SRR33298434.64148 | -0.98% | **RECOMBINANT** | `.AAA-------------------BB------AA-.` |
| chr16R | SRR33298434.378016 | -1.07% | **RECOMBINANT** | `.-AA------BBB------------------AAA.` |
| chr16R | SRR33298377.644709 | -1.08% | **RECOMBINANT** | `.--A-------BB------------------AAAA.` |
| chr16R | SRR33298377.534605 | -1.09% | **RECOMBINANT** | `.AAAA------------------BB-------A--.` |
| chr16R | SRR33298384.248800 | -1.10% | **RECOMBINANT** | `.AAAA------------------BB-------B--` |
| chr16R | SRR33298373.72733 | -1.12% | **RECOMBINANT** | `B-AA------BB-------------------AAA.` |
| chr5L | SRR33298373.260098 | +4.52% | unresolved | `..............................................` |
| chr5L | SRR33298434.226995 | +0.36% | **RECOMBINANT** | `.A--BB------BBB---AA-------A--------A----A.` |
| chr5L | SRR33298434.116145 | +0.33% | unresolved | `...........................................` |
| chr7R | SRR33298434.164214 | +1.13% | **RECOMBINANT** | `.BBBBB---B-BBBB-B--B-AA-A---AAAAAAAAA-A---AA.` |
| chr8R | SRR33298377.221175 | -0.29% | unresolved | `......................................` |

`A` = window favours the read's own (true) element, `B` = window favours the donor element,
`-` = too close to call (<1% apart), `.` = neither aligns. `RECOMBINANT` requires runs of >=2
consecutive decisive windows for BOTH elements on opposite sides of the read (a real crossover);
`unresolved` means too few decisive windows to call a pattern either way -- not evidence against
recombination, just inconclusive; `single` means only one element ever wins, i.e. no crossover
was found in this region at all.

**Result: 17 of 24 (71%) are independently confirmed** -- a genuine A-then-B (or B-then-A)
crossover, not inferred from the global-alignment gap alone. All 7 chr16R and all 6 chr10L
reads confirm cleanly (the two most consistent recipients), so the specific claim made earlier
in this document about those 13 reads holds up under a from-scratch rerun, not just the reused
label. **6 of 24 come back `unresolved`** (chr14R x2, chr16L, chr5L x2, chr8R) -- the true and
donor elements are too similar over the scanned region for this coarse 300bp/150bp window to
resolve a crossover; this is inconclusive, not a negative result, and would need a finer window
or the half-split method (`flank_identities.py`) to settle. **1 of 24 (`SRR33298384.220455`,
chr13L -> G6/chr16R-1) comes back `single`** -- the fresh scan finds no crossover at all, which
does not corroborate the "mid-Y' partial junction" call; this read should be treated as
unconfirmed rather than a supported recombination event.

As a sanity check, the same fresh scan was also run on the 28 "whole-element swap" reads: most
(20/28) come back `single` or `unresolved` rather than `RECOMBINANT`, which is the **expected**
result, not a contradiction -- a true whole-element swap has no internal crossover for a
window scan to find (the donor wins every window, or the two are too similar to call), so
`single`/`unresolved` is exactly what should happen there. That the mid-Y' and whole-element
groups get systematically different fresh-scan outcomes (71% crossover-confirmed vs. mostly no
crossover found) is itself independent support that the global-alignment test is cleanly
separating two real, distinct mechanisms rather than drawing an arbitrary line through one.

## Bottom line: 58 of 58 mismatched reads, true global alignment

| verdict | count | % of 58 | independently window-scan confirmed |
|---|---|---|---|
| whole-element swap (gap > +5%) | 28 | 48% | n/a -- no crossover expected (see above) |
| mid-Y' partial junction (-2% to +5%) | 24 | 41% | 17 confirmed, 6 unresolved, 1 not corroborated |
| not supported (gap < -2%) | 6 | 10% | n/a |

**At least 45 of 58 (78%) reads have a recombination signature that is confirmed by two
independent methods** (28 whole-element swaps by global alignment + 17 mid-Y' junctions
confirmed by a from-scratch sliding-window rerun). A further 6 mid-Y' calls are plausible but
unresolved pending a finer-grained scan, and 1 mid-Y' call did not hold up and should be
dropped from the confirmed count. Only 6 reads (chr12L x2, chr14L x1, chr8L x2, chr2L x1) fail
to clear even the partial-junction bar and should be treated as unconfirmed grouping noise
rather than recombination.

The two mechanistic classes do not track cleanly with any single recipient -- chr13L, for
example, has both whole-element swaps (15/18) and mid-Y' cases (`SRR33298461.44252`, confirmed
by the fresh scan; `SRR33298384.220455`, not corroborated) -- but the split is informative in
aggregate: whole-element swaps dominate where the donor and recipient are close in length and
homology (chr13L->G1, chr2L->G3, chr14R, chr8R), while mid-Y' partial junctions cluster in the
size-mismatched, highly-homologous G8/G7 cases (chr10L->G8, chr16R->G2, chr5L->G11/G7,
chr7R->G10), where all 7 chr16R and all 6 chr10L reads are independently crossover-confirmed by
the from-scratch sliding-window rerun above.
