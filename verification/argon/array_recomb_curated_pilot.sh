...
MANIFEST=$STAGE/recomb_curated_pilot_manifest.txt
...
for f in analyze_features.py aggregate_recombination.py recombination_utils.py; do cp "$STAGE/$f" "$RUNDIR/_pipeline/scripts/$f"; done
...
open(p, "w").write(s); print("y_prime_lib_override ->", lib)
PY
