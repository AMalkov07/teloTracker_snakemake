...
CODE=$STAGE/path
MANIFEST=$STAGE/elem_lib_manifest_7172_7302.txt
...
mkdir -p "$ELEMDIR"
if [ ! -s "$ELEMLIB" ]; then
  python3 "$STAGE/make_element_yprime_lib.py" "$ASM" "$BED" "$ELEMLIB.$$" "$ELEMDIR/elem_${DAY0}.tsv.$$"
  mv -f "$ELEMLIB.$$" "$ELEMLIB"; mv -f "$ELEMDIR/elem_${DAY0}.tsv.$$" "$ELEMDIR/elem_${DAY0}.tsv"
fi
grep -c '^>' "$ELEMLIB"
