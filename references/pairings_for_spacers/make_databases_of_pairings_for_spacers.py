#!/usr/bin/env python3
"""
Make databases for each y_prime group and each pair of y_prime groups for spacer sequences.

Usage:
    python make_databases_of_pairings_for_spacers.py <strain_id> <y_prime_fasta> <spacer_fasta> <output_base_dir>

Example:
    python make_databases_of_pairings_for_spacers.py 7302 \
        references/extracted_yprimes_7302.fasta \
        references/7302_50kb_chopped_up_spacer_sequences.fasta \
...
def get_y_prime_0_ends(y_prime_ids):
    """Get the chromosome ends that have 0 Y' primes, derived from the library."""
    all_chr_ends = set(f'chr{n}{s}' for n in range(1, 17) for s in ('L', 'R'))
    ends_with_yprimes = set()
    for chr_ends in y_prime_ids.values():
        ends_with_yprimes.update(chr_ends)
    return sorted(all_chr_ends - ends_with_yprimes)


def main():
    if len(sys.argv) < 5:
        print("Usage: python make_databases_of_pairings_for_spacers.py <strain_id> <y_prime_fasta> <spacer_fasta> <output_base_dir>")
        print("\nExample:")
        print("  python make_databases_of_pairings_for_spacers.py 7302 \\")
        print("      references/extracted_yprimes_7302.fasta \\")
        print("      references/7302_50kb_chopped_up_spacer_sequences.fasta \\")
        print("      references/pairings_for_spacers/")
        sys.exit(1)

    strain_id = sys.argv[1]
    y_prime_fasta = sys.argv[2]
    spacer_fasta = sys.argv[3]
    ...
    for chr_group_id in y_prime_ids.keys():
        ...
        with open(spacer_fasta, 'r') as all_spacers_file, open(output_file, 'w') as out_file:
            for record in SeqIO.parse(all_spacers_file, 'fasta'):
    ...
    y_prime_0_ends = get_y_prime_0_ends(y_prime_ids)
